...
/*
	...
	This module will need to:
		- interact with raw controller response, expecting Mayflash
		- report entire buffer (26 bits)
		- support for all GCN buttons

	Later:
		- Turbo
		- Threshold cutoff
		- simulated physical magnitude maximum
		- Keyboard input
		- Gcn2NES --> NES2USB --> Input detection?
*/

class GcnController : public BaseControlDevice
{
private:
...
public:
	...
	GcnController(Emulator* emu, ControllerType type, uint8_t port, KeyMappingSet keyMappings) : BaseControlDevice(emu, type, port, keyMappings)
	{
	...
	uint8_t ReadRam(uint16_t addr) override
	{
		...
		if(addr == 0x4016 && IsPressed(GcnController::Buttons::Microphone)) {
			output |= 0x04;
		}
